...
# 配布版（ビルド不要）
.\dist\TXT-TRANS\TXT-TRANS.exe
```

`git lfs pull` で **models/** と **dist/TXT-TRANS/**（約 1.3 GB）を取得します。
...
生成物: `dist\TXT-TRANS\`（one-folder、モデル同梱、約 **1.3 GB**）。  
リポジトリには LFS で `dist/TXT-TRANS/` を同梱しているため、clone + `git lfs pull` 後はビルドなしで exe を実行できます。

## リポジトリ構成

```
TXT-TRANS/
├── dist/TXT-TRANS/   # ビルド済み配布版（Git LFS、約 1.3 GB）
